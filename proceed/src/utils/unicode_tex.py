...
'''
from commons import merge_dicts
...
def specialchars_to_tex(s):
    '''
    Convert special characters of string s to its' corresponding tex equation.
    '''
    return u''.join([specialchars_to_tex_map.get(i, i) for i in unicode(s)])

def unicode_to_tex(s):
    '''
    Convert unicode string s to its' corresponding tex equation.

    >>> print unicode_to_tex( u"Somme accents: å é à ç and Chinese: 瓣" )
    '''
    return u''.join([unicode_to_tex_map.get(i, i) for i in s])

def unicode_to_ipa(s):
    '''
    Convert unicode string s to its' corresponding tipa equation.

    >>> print unicode_to_ipa( u"Somme characters: Ŏ ɐ" )
    '''
    return u''.join([unicode_to_tipa_map.get(i, i) for i in s])

def unicode_to_texipa(s):
    '''
    Convert unicode string s to its' corresponding tex or tipa equation.

    >>> print unicode_to_texipa( u"Somme accents: å é à ç then IPA characters: ɮ Ŏ ɐ" )
    '''
    merged = merge_dicts( unicode_to_tipa_map , unicode_to_tex_map )
    return u''.join([merged.get(i, i) for i in s])
